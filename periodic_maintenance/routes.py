...
def periodic_maintenance():
    ...
    if request.method == "POST":
        ...
        logger.debug(f"Alınan filtre tipi: {filter_type}")
        ...
        logger.debug(f"Normalize edilmiş filtre tipi: {filter_type}")
...
def generate_offer_pdf(offer_id):
    ...
    try:
        ...
        for maintenance in maintenances:
            if offer.filter_type == "original":  # Düzeltilmiş kontrol
                price_eur = maintenance.original_price_eur
                part_code = maintenance.filter_part_code
            else:
                price_eur = maintenance.alternate_price_eur
            ...
            if price_eur and part_code:
                ...
                parts_table_content += f"""
                    <tr class="{'alternate-row' if row_number % 2 == 0 else ''}">
                        <td>{row_number}</td>
                        <td>{part_code}</td>
                        <td>{maintenance.filter_name}</td>
                        <td>1</td>
        ...
        if offer.parts:
            for part in offer.parts:
                price_try = part.get('price_eur', 0) * eur_to_try
        ...
        if offer.oils:
            for oil in offer.oils:
                ...
                price_try = oil.get('price_eur', 0) * eur_to_try
    ...
    if request.method == "POST":
        try:
            ...
            for key, value in request.form.items():
                if key.startswith("part_quantity_"):
                    part_id = int(key.split("_")[-1])
                    quantity = int(value)
                    if quantity > 0:
                        part = PeriodicMaintenance.query.get(part_id)
                        if part:
                            if filter_type == "original":
                                price_eur = part.original_price_eur
                                part_code = part.filter_part_code
                            else:
                                price_eur = part.alternate_price_eur
            ...
            # Toplam maliyetleri hesapla (Euro cinsinden)
            parts_total_eur = sum(part["total_eur"] for part in selected_parts)
            oils_total_eur = sum(oil["total_eur"] for oil in selected_oils)
            labor_cost_eur = labor_cost / eur_rate
            travel_cost_eur = travel_cost / eur_rate
            subtotal_eur = parts_total_eur + oils_total_eur + labor_cost_eur + travel_cost_eur

            # İskonto hesapla (TL cinsinden)
            subtotal_tl = subtotal_eur * eur_rate
            if discount_type == "percentage":
