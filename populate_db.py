...
from models import (
    ...
    QRCode, Warranty, Invoice, PeriodicMaintenance, Offer
)
            ...
            if not Warranty.query.first():
                ...
                warranties = [
                    Warranty(
                        ...
                        end_date=datetime.now(timezone.utc).replace(year=datetime.now().year + 2),
                        description="2 yıllık garanti",
                        document_url="Uploads/warranty_sn12345.pdf",
                        created_by=admin_user.id,
                        created_at=datetime.now(timezone.utc)
                    )
                ]
                db.session.add_all(warranties)
                db.session.commit()
                print("Warranty tablosuna veriler eklendi.")
            else:
